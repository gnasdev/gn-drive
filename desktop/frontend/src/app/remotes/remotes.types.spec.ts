...
describe("RemoteTypes", () => {
    describe("REMOTE_TYPE_OPTIONS", () => {
        it("should contain all expected remote types", () => {
            ...
            const actualTypes = REMOTE_TYPE_OPTIONS.map(
                (option) => option.value,
            );
        ...
        it("should have proper labels for new providers", () => {
            const gPhotosOption = REMOTE_TYPE_OPTIONS.find(
                (opt) => opt.value === "gphotos",
            );
            const iCloudOption = REMOTE_TYPE_OPTIONS.find(
                (opt) => opt.value === "iclouddrive",
            );
    ...
    describe("isValidRemoteType", () => {
        ...
        it("should return false for invalid remote types", () => {
            ...
            invalidTypes.forEach((type) => {
                expect(isValidRemoteType(type as string)).toBe(false);
            });
        });
    ...
    describe("isRemoteFormData", () => {
        ...
        it("should return false for invalid RemoteFormData", () => {
            ...
            invalidData.forEach((data, index) => {
                expect(isRemoteFormData(data))
                    .withContext(
                        `Item at index ${index}: ${JSON.stringify(data)}`,
                    )
                    .toBe(false);
            });
        });
        ...
        it("should validate new provider types correctly", () => {
            expect(isRemoteFormData({ name: "test", type: "gphotos" })).toBe(
                true,
            );
            expect(
                isRemoteFormData({ name: "test", type: "iclouddrive" }),
            ).toBe(true);
        });
    });
