...
import { CommandDTO, SyncEvent } from "./models/events";
import {
    DEFAULT_SYNC_STATUS,
    isValidSyncAction,
    isValidSyncStatus,
...
export class TabService {
    ...
    handleCommandEvent(data: CommandDTO): void {
        ...
        switch (data.command) {
            ...
            case "sync_status":
                ...
                this.handleTabSyncStatusUpdate(
                    data.tab_id,
                    data as SyncStatusEvent,
                );
                break;
        }
    }
    ...
    // Note: sync:progress events are now handled by LogConsumerService for reliable delivery
    handleTypedSyncEvent(event: SyncEvent): void {
        ...
        switch (event.type) {
            case "sync:started":
                ...
                break;
            case "sync:progress":
                // FIXED: Append logs instead of replacing them
                // This prevents log loss when multiple progress events arrive
                if (event.message) {
                    this.updateTab(event.tabId, {
                        data: this.limitOutput([...tab.data, event.message]),
                    });
                }
                break;
            case "sync:completed":
                this.updateTab(event.tabId, {
                    ...
                    isStopping: false,
                    syncStatus: null,
                    data: this.limitOutput([
                        ...tab.data,
                        `Sync completed: ${event.message || ""}`,
                    ]),
                });
                break;
            case "sync:failed":
